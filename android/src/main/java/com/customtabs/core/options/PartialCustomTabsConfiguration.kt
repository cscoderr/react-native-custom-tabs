package com.customtabs.core.options
  ...
  class Builder {
    ...
    fun setOptions(options: Map<String, Any>?): Builder {
      ...
      activityHeightResizeBehavior =
        (options[KEY_ACTIVITY_HEIGHT_RESIZE_BEHAVIOR] as Number?)?.toInt()
      activityHeightResizeBehavior =
        (options[KEY_ACTIVITY_HEIGHT_RESIZE_BEHAVIOR] as Number?)?.toInt()
      ...
      activitySideSheetPosition =
        (options[KEY_ACTIVITY_SIDE_SHEET_POSITION] as Number?)?.toInt()
      activitySideSheetDecorationType =
        (options[KEY_ACTIVITY_SIDE_SHEET_DECORATION_TYPE] as Number?)?.toInt()
      activitySideSheetRoundedCornersPosition =
        (options[KEY_ACTIVITY_SIDE_SHEET_ROUNDED_CORNERS_POSITION] as Number?)?.toInt()
      backgroundInteractionEnabled = options[KEY_BACKGROUND_INTERACTION_ENABLED] as Boolean?
      cornerRadius = (options[KEY_CORNER_RADIUS] as Number?)?.toInt()
      return this
    }
